...
public class PersonControllerTest {
    ...
    public void returnSuccessWhenSaveOk(){
        ...
        TestObserver<Void> testObserver = personController.save(personRequest).test();
    ...
    public void returnPerfilResponseWhenIdPersonOk(){
        ...
        when(personService.getById(ID_PERSON)).thenReturn(Single.just(personResponse));
        TestObserver<PersonResponse> testObserver = personController.getById(ID_PERSON).test();
        testObserver.awaitTerminalEvent();
        testObserver.assertComplete();
        testObserver.assertNoErrors();
    ...
    public void returnPerfilResponseWhenDocumentNumberOk(){
        PersonResponse personResponse = GSON.fromJson(TestUtil.readJsonFileToString("mock/person_request.json"),
                PersonResponse.class);
        when(personService.getByNumberDocument(DOCUMENT_NUMBER)).thenReturn(Maybe.just(personResponse));
        TestObserver<PersonResponse> testObserver = personController.getByNumberDocument(DOCUMENT_NUMBER).test();
        testObserver.awaitTerminalEvent();
        testObserver.assertComplete();
        testObserver.assertNoErrors();
        var actual = testObserver.values().get(0);
